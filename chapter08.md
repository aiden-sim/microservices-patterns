...
- API 게이트웨이 구현 방법
  ...
  - API 게이트웨이 프레임워크 또는 웹 프레임워크를 기반으로 API 게이트웨이 직접 개발 
...
- 처음 구성만 잘 해두면 확장 등 나머지 일은 모두 AWS가 알아서 처리

- 단점
  - API 조합을 지원하지 않기 때문에 직접 백엔드 서비스에 조합 로직 구현 

#### AWS 애플리케이션 부하 분산기
- AWS ALB는 HTTP, HTTPS, 웹 소켓, HTTP2/용 부하 분산기

- 단점
  - 기능이 제한적이고 HTTP 메서드 기반의 라우팅, API 조합, 인증 같은 로직은 없음 

#### 다른 API 게이트웨이 제품
- 콩, 트래픽 등 직접 설치/운영할 수 있는 오픈 소스 제품
- API 조합은 여전히 미지원 
...
- API 조합이 구현된 스프링 MVC 컨트롤러를 정의하면 확장도 가능

- 경로 기반의 라우팅만 지원되는 한계도 있음

#### 스프링 클라우드 게이트웨이
- 스프링 클라우드 게이트웨이는 스프링5, 스프링 부트2, 스프링 웹플럭스 등의 프레임워크를 토대로 한 API 게이트웨이 프레임워크
- 리액터 프로젝트는 Mono 추상체를 제공하는 NIO 기반의 JVM 리액티브 프레임워크

- 다음과 같이 단순하지만 범용적인 수단 제공
  - 요청을 백엔드 서비스로 보냄
...
#### OrderConfiguration
- OrderConfiguration은 `/orders` 끝점을 구현한 스프링 빈이 정의된 스프링 구성 클래스
- 스프링 웹 플럭스 라우팅 DSL로 정의된 요청 라우팅 규칙은 orderProxyRouting, orderHandlerRouting 빈에 있음

#### OrderHandlers 클래스
- OrderHandlers 클래스에는 API 조합을 비롯한 사용자 정의 로직이 구현된 요청 핸들러 메서드가 있음
- 백엔드 서비스에 실제로 요청하는 여러 프록시 클래스가 여기에 주입

- getOrderDetails 메서드 수행
  - 1) orderId를 경로에서 추출
  - 2) 프록시를 통해 네 서비스를 비동기로 호출
  - 3) 결과를 값이 4개 포함된 `Mono<Tuple4>` 을 반환하는 `Mono.when()` 으로 비동기 조합
  - 4) 스프링 웹플럭스에서 JSON/HTTP 응답을 나타내는 ServerResonse로 OrderDetails를 변환


#### OrderServiceProxy 클래스
- OrderServiceProxy는 주문 서비스용 원격 프록시 클래스로 WebClient (스프링 웹플럭스의 리액티브 HTTP 클라이언트)로 주문 서비스 호출

#### ApiGatewayApplication 클래스
- 스프링 클라우드 게이트웨이는 라우팅 규칙을 DSL 형태로 구성 가능
- 스프링 클라우드 게이트웨이는 확장 가능하면서 리액티브한 스프링5와 리액터 프로젝트 프레임워크가 잘 버무려진 작품

### 8.3.3 API 게이트웨이 구현: GraphQL
- 그래프 기반 API 프레임워크는 그래프 기반의 스키마로 서버 API를 구성하는 것이 핵심
- 클라이언트는 그래프 노드와 이들의 프로퍼티/관계 단위로 필요한 데이터를 지정해서 조회하기 때문에 API 게이트웨이로 원하는 데이터를 한번에 조회 가능

- 장점
  - 1) 클라이언트가 반환 데이터를 제어가능
  - 2) 개발 수고를 줄일 수 있음 

- GraphQL의 설계 핵심 요소
  - GraphQL 스키마 : 서버 쪽 데이터 모델 및 이 모델이 지원하는 쿼리 정의
...
- 스키마에는 다음 세 쿼리가 정의되어 있음
  - orders() : 주어진 Customer가 주문한 여러 Order를 반환
  - order() : 주어진 Order를 반환
  - consumer() : 주어진 Consumer를 반환

#### GraphQL 쿼리 실행
- GraphQL의 가장 큰 장점은 클라이언트가 반환 데이터를 쿼리 언어로 자유롭게 제어가능
- 쿼리는 Query 객체의 필드고, 이 객체의 어떤 필드를 서버가 반환해야 할지 쿼리 문서에 지정하는 구조

#### 스키마를 데이터에 연결
- GraphQL 서버는 쿼리 실행 시 하나 이상의 데이터 저장소에서 요청한 데이터를 가져와야 함
- 스키마에 정의된 객체형 필드에 리졸버 함수를 붙이면 GraphQL 스키마를 데이터 소스와 연관 지을 수 있음

- 리졸버 함수는 다음 세 매개변수를 받음
  - 객체 : (이 부분 이해를 못함)
  - 쿼리 인수 : 쿼리 문서에 명시된 값들
  - 컨텍스트 : 모든 리졸버가 접근 가능한 전역 쿼리 실행 상태

- 아폴로 GraphQL 서버의 리졸버 함수는 자바 CompletableFuture의 자바스크립트 버전에 해당하는 프라미스를 반환
  - 프라미스에는 리졸버 함수가 데이터 저장소에서 가져온 객체가 포함되어 있음

- GraphQL은 재귀 알고리즘을 이용하여 리졸버 함수를 실행

#### 배치/캐싱으로 로딩 최적화
- GraphQL 쿼리 실행 시, 많은 리졸버를 실행하면 성능이 떨어질 위험이 있음
- 성능을 최적화하기 위해 서버 쪽에서 배칭/캐싱을 조합하여 사용

- 배칭
  - 서비스를 N번 호출할 것을 N개 객체를 일괄 조회하는 호출 한번으로 전환

- 캐싱
  - 불필요한 중복 호출을 방지하여 동일한 객체는 이전 결과를 재사용 

#### 아폴로 GraphQL 서버와 익스프레스를 연계
- 클라이언트가 GraphQL 서버를 호출하려면 웹 프레임워크에 연계해야 함

#### GraphQL 클라이언트 작성
- 클라이언트는 적당한 HTTP 라이브러리를 이용하여 요청하면 됨
  - 클라이언트 쪽 캐싱 등의 기능을 제공하는 GraphQL 클라이언트 라이브러리를 사용하는 것이 더 쉬움 
