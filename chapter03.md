...
- REST 는 사용하기 쉽고 대중적인 IPC 지만, 동기 프로토콜이기 때문에 호출한 서비스가 응답할 때까지 HTTP 클라이언트가 기다려야 함
  - 가용성 저하

- ![스크린샷 2022-04-18 오후 12 01 17](https://user-images.githubusercontent.com/7076334/163747717-4a3fd2d4-9acb-4eab-87ce-4cfe3e53087c.png)
  - 세 서비스 모두 HTTP를 사용하기 때문에 주문 생성 요청이 정상 처리되려면 세 서비스 모두 가동 중 이어야 함
  - 한 서비스라도 내려가면 주문 생성 불가
  - 더 많은 서비스가 요청 처리에 개입할 수록 가용성은 더 낮아짐
  - 가용성을 최대화 하려면 동기 통신을 최소화 해야함

### 3.4.2 동기 상호 작용 제거

#### 비동기 상호 작용 스타일
- ![스크린샷 2022-04-18 오후 12 06 03](https://user-images.githubusercontent.com/7076334/163748151-5fdeaae5-2720-4ed7-952d-a8c788e88129.png)
  - 클라이언트/서비스는 메시징 채널을 통해 메시지를 전송해서 서로 비동기 통신 (응답을 대기하며 블로킹되지 않음)
  - 주문 응답 생성 묶는 방법 고려해야됨

- 하지만 지연 없이 즉시 응답해야 하는 경우는?
  - 데이터 복제 사용 

#### 데이터 복제
- 서비스 요청 처리에 필요한 데이터의 레플리카를 유지하는 방법
- 데이터 레플리카는 서비스 발행 이벤트를 구독해서 최신 데이터로 유지 가능

- ![스크린샷 2022-04-18 오후 12 10 37](https://user-images.githubusercontent.com/7076334/163748497-987bf73d-8a24-46c9-945a-b7952567a9a4.png)
  - 주문 생성 요청 시, 소비자 서비스, 음식적 서비스 조회할 필요 없다.
  
- 하지만 대용량 데이터의 레플리카를 만드는 것은 대단히 비효율적
  - 해결법은 자신의 클라이언트에 응답하기 전까지 다른 서비스와의 상호 작용을 지연시키는 것

#### 응답 반환 후 마무리
- 요청 처리 도중 동기 통신을 제거하려면 요청을 다음과 같이 처리하면 됨
  - 1) 로컬에서 가용한 데이터만 갖고 요청을 검증
  - 2) 메시지를 OUTBOX 테이블에 삽입하는 식으로 DB를 업데이트 함
  - 3) 클라이언트에 응답을 반환
    - 서비스에 메시지를 비동기 전송 (느슨하게 결합)  

- ![스크린샷 2022-04-18 오후 12 14 23](https://user-images.githubusercontent.com/7076334/163748791-f96df804-258d-45cb-acaf-1209d4ea77e3.png)
  - 내용은 150P 참고
  - 주문 서비스는 응답 반환 시 새로 생성된 주문 상태에 관한 최소한의 정보만 보장 (검증이나 신용카드 승인 상태는 아직 완료전)
  - 클라이언트 입장에서 주문 생성 성공 여부를 알아내려면 주기적으로 폴링하거나 주문 서비스가 알림 메시지를 보내 주어야 함(FCM)
